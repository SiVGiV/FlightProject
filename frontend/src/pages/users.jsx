...
function UserList({ users, userType, forceReload }) {
    ...
    return (
        <div className="userGrid">
            <>
                <div className="columnHeader column-1">User ID</div>
                <div className="columnHeader column-2">Username</div>
                <div className="columnHeader column-3">Name</div>
                <div className="columnHeader column-4">E-Mail</div>
...
function User({ profile }) {
    ...
    return (
        <>
            <div
                className={"userItem column-1" + (!active ? " cancelled" : "")}
            >
                {profile.userid}
            </div>
            <div
                className={"userItem column-2" + (!active ? " cancelled" : "")}
            >
                {profile.username}
            </div>
            <div
                className={"userItem column-3" + (!active ? " cancelled" : "")}
            >
                {profile.name}
            </div>
            <div
                className={"userItem column-4" + (!active ? " cancelled" : "")}
            >
                {profile.email}
            </div>
            <div
                className={"userItem column-5" + (!active ? " cancelled" : "")}
            >
                {profile.entityid}
            </div>
            <div
                className={"userItem column-6" + (!active ? " cancelled" : "")}
            >
                <ValidationButton
                    onClick={handleCancel}
                    variant="danger"
                    disabled={login.id === profile.userid || !active}
                    text="Deactivate"
                />
            </div>
        </>
