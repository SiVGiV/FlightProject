...
function RouteResolver(page, index) {
    if ("url" in page && "element" in page) {
        return (
            <Route key={index} path={page["url"]} element={page["element"]} />
        );
    }
    if (typeof Object.values(page)[0] == "object") {
        return Object.values(page).map((p, ind) => (
            <Route key={ind} path={p["url"]} element={p["element"]} />
        ));
    }
}
